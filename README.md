...
- A separate server worker claims the durable MySQL job, transcribes with `gpt-4o-mini-transcribe`, analyzes with `gpt-4o`, and writes audit artifacts without changing the original audio. The prompt reviews every questionnaire question twice. After the initial answer list, the server audits every missing questionnaire ID in one additional pass, recognizes safe shortened forms such as omitted parenthetical explanations, narrows deterministic searches between neighboring matched questions, and accepts model-only recovery only when its exact evidence exists in the transcript. A located question that still lacks a usable extraction becomes a clarification request instead of disappearing; an unasked question with no verified evidence remains absent.
...
`session.json` is created only after analysis is accepted or all clarification requests are resolved. It records `metadata.processing.analysis_pipeline_version` and a top-level `completeness_check` audit containing the initially missing, deterministically located, recovered, clarification-placeholder, unasked, and rejected-unverified question IDs. Follow-up answers remain nested under their stable parent question ID, while the native and web dashboards label them as `Q# Follow-up`; SQL analysis indexes them separately as `<question-id>:follow_up`. The original `.m4a` is never replaced by a derived file.
